  ...
  state: {
    ...
    products: [
      ...
      {name: 'Смарт-часы Fossil SPORT FTW6051',
       ...
       img_slider: [
        {img:'fossil.png', id: 1}, 
        {img:'fossil2.png', id: 2},
        {img:'fossil3.png', id: 3}
       ],
      ...
      {name: 'Смарт-часы Geozon Titanium',
       ...
       img_slider: [
        {img:'geozon.png', id: 1}, 
        {img:'geozon2.png', id: 2},
        {img:'geozon3.png', id: 3}
       ],
      ...
      {name: 'Смарт-часы Amazfit GTR 47mm Titanium',
       ...
       prise: 15000,
       img_slider: [
        {img:'amaxfit.png', id: 1}, 
        {img:'amaxfit2.png', id: 2},
        {img:'amaxfit3.png', id: 3}
       ],
  ...
  mutations: {
    addToBasket(state, {product}){
      if(state.shop.find(item => item.id === product.id)){
        product.quantity ++;
        product.totalprise = product.prise * product.quantity;
      }else{
        state.shop.push(product);
        product.quantity = 1;
        product.totalprise = product.prise;
      }
      state.count ++;
    ...
    getLess(state, {product}){
      ...
      if(index>-1){
          if(state.shop[index].quantity == 0){
            ...
            state.shop[index].totalprise = state.shop[index].prise * state.shop[index].quantity;
    ...
    getMore(state, {product}){
      ...
      if(index>-1){
          state.shop[index].quantity += 1;
          state.totalSum -= state.shop[index].totalprise;
          state.shop[index].totalprise = state.shop[index].prise * state.shop[index].quantity;
          state.totalSum += state.shop[index].totalprise;
          state.count += 1;
      }
    },
    removeFromShopItem(state, {product}) {
      let index = state.shop.indexOf(product);
      if (index > -1) {
          state.count -= state.shop[index].quantity;
          state.totalSum -= state.shop[index].totalprise;
          state.shop.splice(index, 1);
      }
  },
    removeFromShop(state) {
      
      state.count = 0;
      state.totalSum = 0;
      while(state.shop.length){
        state.shop.pop();
      }
  },
    getTotalSum(state){
      for(let item of state.shop){
        state.totalSum += item.totalprise;
      }
